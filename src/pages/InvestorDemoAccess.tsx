...
import { Shield, Mail, Key, ArrowRight, AlertCircle } from "lucide-react";
import { investorDemoApi } from "@/lib/api-client";

export default function InvestorDemoAccess() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await investorDemoApi.verifyAccess(accessCode, email);
      ...
      // Store access info in sessionStorage for demo portal
      sessionStorage.setItem("investor_demo_session", JSON.stringify({
        accessId: response.data.access.id,
        email: response.data.access.email,
        fullName: response.data.access.full_name,
        organization: response.data.access.organization,
        expiresAt: response.data.access.expires_at,
      }));

      navigate("/investor-demo");
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-muted">
      ...
      <div className="mx-auto max-w-7xl px-6 pt-8 pb-4">
        <p className="mx-auto max-w-xl text-center text-base text-black sm:text-lg md:text-xl">
          Enter your invitation code and email to access the demo portal
        </p>
      </div>
