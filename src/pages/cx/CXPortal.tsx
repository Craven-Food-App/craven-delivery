...
export default function CXPortal() {
  ...
  const StatusPill = ({ status }: { status: string }) => {
    const map: Record<string, string> = {
      ...
      posted: "bg-amber-500/15 text-amber-300 border-amber-500/40",
      offered: "bg-blue-500/15 text-blue-300 border-blue-500/40",
      accepted: "bg-indigo-500/15 text-indigo-300 border-indigo-500/40",
      en_route_pickup: "bg-indigo-500/15 text-indigo-300 border-indigo-500/40",
      picked_up: "bg-purple-500/15 text-purple-300 border-purple-500/40",
      en_route_dropoff: "bg-purple-500/15 text-purple-300 border-purple-500/40",
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-mono uppercase tracking-wider border ${map[status] ?? "bg-slate-700 text-slate-300 border-slate-600"}`}>
        {status.replace(/_/g," ")}
      </span>
    );
  };
  ...
  const Kpi = ({ label, value, sub, icon: Icon, accent, trend }: any) => (
    <div className="relative bg-[#111A2E] border border-slate-800 rounded-lg p-3 overflow-hidden">
      ...
      <div className="mt-1.5 text-xl font-bold text-white tabular-nums leading-none">{value}</div>
      {sub && (
  ...
  const PipelineColumn = ({ title, items, accent, icon: Icon }: any) => {
    ...
    return (
      <div className="bg-[#0F1729] border border-slate-800 rounded-lg flex flex-col min-h-[280px]">
        <div className={`flex items-center justify-between px-3 py-2 border-b border-slate-800 ${accent}`}>
          <div className="flex items-center gap-2">
            <Icon className="h-3.5 w-3.5"/>
            <span className="text-[11px] font-bold uppercase tracking-wider">{title}</span>
          </div>
          <span className="text-[11px] font-mono tabular-nums bg-black/30 px-1.5 py-0.5 rounded">{items.length}</span>
        </div>
        <div className="p-2 space-y-1.5 flex-1 overflow-auto max-h-[420px]">
          {items.length === 0 && (
            <div className="text-center py-8 text-[11px] text-slate-600 font-mono">— EMPTY —</div>
          )}
          {items.map((j:any) => {
            const s = stops(j);
            ...
            return (
              <div key={j.id} className="bg-[#0B1220] border border-slate-800 hover:border-orange-500/50 rounded p-2 text-xs cursor-pointer transition">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-mono text-[10px] text-orange-400">#{j.id.slice(0,6).toUpperCase()}</span>
                  <StatusPill status={j.status}/>
                </div>
                <div className="text-slate-300 flex items-center gap-1 truncate">
                  <MapPin className="h-3 w-3 text-orange-500 shrink-0"/>
                  <span className="truncate text-[11px]">{pickup?.address ?? "—"}</span>
                </div>
                <div className="text-slate-300 flex items-center gap-1 truncate mt-0.5">
                  <ArrowRight className="h-3 w-3 text-slate-600 shrink-0"/>
                  <span className="truncate text-[11px]">{dropoff?.address ?? "—"}</span>
                </div>
                <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-slate-800/60">
    ...
    <div className="min-h-screen bg-[#0B1220] text-slate-200 flex">
      ...
      <aside className={`${navOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:static inset-y-0 left-0 z-40 w-60 bg-[#070D1A] border-r border-slate-800 flex flex-col transition-transform`}>
        ...
        <div className="px-3 py-3 border-b border-slate-800">
          <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-1">Operator</div>
          <div className="text-sm font-semibold text-white truncate">{restaurant.name}</div>
          <div className="mt-2 flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"/>
            <span className="text-[10px] text-emerald-400 font-mono uppercase tracking-wider">Online · Live</span>
          </div>
        </div>
          ...
          {nav_items.map(([k, label, Icon]) => {
            ...
            return (
              ...
              >
                ...
                {badge !== null && badge > 0 && (
                  <span className={`text-[10px] font-mono tabular-nums px-1.5 py-0.5 rounded ${isActive ? "bg-orange-500 text-white" : "bg-slate-800 text-slate-400"}`}>{badge}</span>
                )}
              </button>
            );
          })}
        ...
        <div className="p-3 border-t border-slate-800">
          <button
            ...
            className="w-full flex items-center gap-2 px-2.5 py-2 rounded text-[13px] text-slate-400 hover:bg-rose-500/10 hover:text-rose-400 transition"
          >
      ...
      {/* Overlay */}
      {navOpen && <div onClick={() => setNavOpen(false)} className="fixed inset-0 bg-black/60 z-30 lg:hidden"/>}

      {/* Main */}
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header className="bg-[#070D1A] border-b border-slate-800 px-4 py-2.5 flex items-center justify-between gap-3 sticky top-0 z-20">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="hidden sm:flex items-center gap-2 text-[11px] font-mono">
              ...
              <span className="text-orange-400 uppercase">{tab}</span>
            </div>
          </div>
          <div className="flex items-center gap-3 text-[11px] font-mono">
            ...
            <div className="text-slate-400 tabular-nums">
              {now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", hour12: false })} <span className="text-slate-600">LOCAL</span>
            </div>
            <button className="relative text-slate-400 hover:text-orange-400">
        ...
        <main className="flex-1 overflow-auto">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-bold text-white uppercase tracking-wider">Live Pipeline</h2>
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"/>
                  </div>
                  <span className="text-[10px] font-mono text-slate-500">REALTIME · SUPABASE</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
                  ...
                  <PipelineColumn title="Delivered" items={deliveredToday} icon={CheckCircle2} accent="text-emerald-400"/>
                </div>
              </div>

              {/* Secondary row */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                <div className="lg:col-span-2 bg-[#111A2E] border border-slate-800 rounded-lg">
                  <div className="px-3 py-2 border-b border-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <History className="h-3.5 w-3.5 text-slate-400"/>
                      <span className="text-[11px] font-bold uppercase tracking-wider text-slate-300">Recent Activity</span>
                    </div>
                    <button onClick={() => setTab("history")} className="text-[11px] text-orange-400 hover:underline">View all →</button>
                  </div>
                  <div className="p-2">
                ...
                <div className="bg-[#111A2E] border border-slate-800 rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-3">
                    <BarChart3 className="h-3.5 w-3.5 text-slate-400"/>
                    <span className="text-[11px] font-bold uppercase tracking-wider text-slate-300">Performance</span>
                  </div>
                  <div className="space-y-3 text-sm">
                    <div>
                      <div className="flex justify-between text-[11px] text-slate-400 mb-1">
                        <span>Avg Ticket</span><span className="text-white font-bold tabular-nums">${avgTicket.toFixed(2)}</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] text-slate-400 mb-1">
                        <span>Success Rate</span><span className="text-white font-bold tabular-nums">{successRate.toFixed(1)}%</span>
                      </div>
                      <div className="h-1 bg-slate-800 rounded overflow-hidden">
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] text-slate-400 mb-1">
                        <span>Failure Rate</span><span className="text-white font-bold tabular-nums">{jobs.length ? ((failed.length/jobs.length)*100).toFixed(1) : "0.0"}%</span>
                      </div>
                      <div className="h-1 bg-slate-800 rounded overflow-hidden">
                    ...
                    <div className="pt-2 border-t border-slate-800 grid grid-cols-2 gap-2 text-center">
                      <div>
                        ...
                        <div className="text-base font-bold text-white tabular-nums">{todayJobs.length}</div>
                      </div>
                      <div>
                        <div className="text-[10px] uppercase tracking-wider text-slate-500">Lifetime</div>
                        <div className="text-base font-bold text-white tabular-nums">{jobs.length}</div>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {tab === "active" && (
            <div className="p-4">
              <div className="mb-3 flex items-center gap-2">
                <Radio className="h-4 w-4 text-orange-500"/>
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Active Operations</h2>
                <span className="text-[10px] font-mono text-slate-500">({active.length})</span>
              </div>
          ...
          {tab === "history" && (
            <div className="p-4">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">Mission History</h2>
                <span className="text-[10px] font-mono text-slate-500">({completed.length})</span>
              </div>
          ...
          {tab === "fleet" && (
            <div className="p-4">
              <div className="bg-[#111A2E] border border-slate-800 rounded-lg p-8 text-center">
                <Users className="h-10 w-10 mx-auto text-slate-600 mb-3"/>
                <h3 className="text-base font-bold text-white">Fleet Roster</h3>
          ...
          {tab === "analytics" && (
            <div className="p-4">
              <div className="bg-[#111A2E] border border-slate-800 rounded-lg p-8 text-center">
                <BarChart3 className="h-10 w-10 mx-auto text-slate-600 mb-3"/>
                <h3 className="text-base font-bold text-white">Operational Analytics</h3>
