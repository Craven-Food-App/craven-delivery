...
import { CxoTrainingLessonWithDetails, CxoTrainingProgress } from '@/types/cxo-training';
...
const CxoTrainingLesson: React.FC = () => {
  const { lessonId } = useParams<{ lessonId: string }>();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [lesson, setLesson] = useState<CxoTrainingLessonWithDetails | null>(null);
  const [progress, setProgress] = useState<CxoTrainingProgress | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [lessonData, progressData] = await Promise.all([
        trainingRepository.getLessonWithDetails(lessonId),
        trainingRepository.getProgressForLesson(user.id, lessonId),
      ]);
      ...
      if (!progressData) {
        await trainingRepository.upsertProgress(user.id, lessonData?.moduleId || null, lessonId, {
          status: 'in_progress',
          lastAccessedAt: new Date().toISOString(),
        });
        await trainingRepository.createAuditEntry(
          user.id,
          'lesson_started',
  ...
  const handleStepToggle = async (stepId: string, completed: boolean) => {
    ...
    await trainingRepository.markStepCompleted(userId, lessonId, stepId);
    await trainingRepository.createAuditEntry(userId, 'step_completed', lesson?.moduleId || null, lessonId, {
      stepId,
    });
  ...
  const handleSubmitQuiz = async () => {
    ...
    await trainingRepository.upsertProgress(userId, lesson.moduleId, lesson.id, {
      quizScore: score,
    });

    await trainingRepository.createAuditEntry(userId, 'quiz_submitted', lesson.moduleId, lesson.id, {
      score,
      correctCount,
  ...
  const handleCompleteLesson = async () => {
    ...
    const success = await trainingRepository.upsertProgress(userId, lesson.moduleId, lesson.id, {
      status: 'completed',
      completedAt: new Date().toISOString(),
    });

    if (success) {
      await trainingRepository.createAuditEntry(userId, 'lesson_completed', lesson.moduleId, lesson.id);
      notifications.show({
      ...
      navigate(`/cxo/training/modules/${lesson.moduleId}`);
    }
  };
    ...
    <Stack gap="lg">
      <Group>
        <Button
          ...
          onClick={() => navigate(`/cxo/training/modules/${lesson.moduleId}`)}
        >
          Back to Module
        </Button>
      </Group>
...
export default CxoTrainingLesson;
