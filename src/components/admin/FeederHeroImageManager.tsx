...
export const FeederHeroImageManager: React.FC = () => {
  ...
  const fetchHeroImage = async () => {
    try {
      ...
      const imageUrl = data?.feeder_hero_image_url || '';
      setHeroImageUrl(imageUrl);
      setImagePreview(imageUrl || null);
    } catch (error: any) {
      console.error('Error fetching feeder hero image:', error);
      toast({
        title: 'Error',
        description: 'Failed to fetch feeder hero image settings',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      const fileName = `feeder-hero/${Date.now()}.${fileExt}`;
    ...
    try {
      ...
      if (existing) {
        ...
        const { error: updateError } = await supabase
          .from('marketing_settings')
          .update({
            feeder_hero_image_url: heroImageUrl || null,
            updated_by: user.id,
          })
          .eq('id', existing.id);
        error = updateError;
      ...
      toast({
        title: 'Success',
        description: 'Feeder hero image updated successfully',
      });

      fetchHeroImage();
    } catch (error: any) {
      console.error('Error saving feeder hero image:', error);
      toast({
        title: 'Error',
        description: error.message || 'Failed to save feeder hero image',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
            ...
            {!imagePreview ? (
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-gray-400 transition-colors">
                <input
                  ref={fileInputRef}
                  type="file"
                  id="feeder-hero-image"
                  accept="image/*"
                  onChange={handleFileSelect}
                  className="hidden"
                  disabled={uploadingImage}
                />
                <label htmlFor="feeder-hero-image" className="cursor-pointer">
