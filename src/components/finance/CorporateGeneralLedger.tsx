...
interface CorporateGeneralLedgerProps {
  onHeaderActionsChange?: (actions: React.ReactNode | null) => void;
}

export const CorporateGeneralLedger: React.FC<CorporateGeneralLedgerProps> = ({ onHeaderActionsChange }) => {
  ...
  const openCreateEntryModal = () => {
    if (!canManage) {
      message.warning('You need Finance Department access to create journal entries');
      return;
    }
  ...
  useEffect(() => {
    ...
    onHeaderActionsChange(
      <Space>
        ...
        <Button icon={<ReloadOutlined />} onClick={fetchGLData}>
          Refresh
        </Button>
        <Button
          type="primary"
  ...
  }, [onHeaderActionsChange, canManage, filteredTransactions.length]);
