...
import { CtoTrainingModule, CtoTrainingLesson, CtoTrainingProgress } from '@/types/cto-training';
import { LessonListItem } from './LessonListItem';
import { IconArrowLeft, IconClock, IconExternalLink, IconArrowRight } from '@tabler/icons-react';
import { supabase } from '@/integrations/supabase/client';

const CtoTrainingModuleDetail: React.FC = () => {
  const { moduleId: paramModuleId } = useParams<{ moduleId: string }>();
  const location = useLocation();
  const navigate = useNavigate();
  
  // Extract moduleId from URL if not in params (for CTO Portal routing)
  const moduleId = paramModuleId || location.pathname.split('/training/modules/')[1]?.split('/')[0] || null;
  const [loading, setLoading] = useState(true);
  const [module, setModule] = useState<CtoTrainingModule | null>(null);
  const [lessons, setLessons] = useState<CtoTrainingLesson[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [moduleData, lessonsData, progressData] = await Promise.all([
        ctoTrainingRepository.getModuleById(moduleId),
        ctoTrainingRepository.getLessonsByModuleId(moduleId),
      ...
      if (!moduleProgress) {
        await ctoTrainingRepository.upsertProgress(user.id, moduleId, null, {
          status: 'in_progress',
          lastAccessedAt: new Date().toISOString(),
        });
        await ctoTrainingRepository.createAuditEntry(user.id, 'module_started', moduleId, null);
      }
    } catch (error) {
  ...
  const handleStartLesson = (lessonId: string) => {
    navigate(`/cto/training/lessons/${lessonId}`);
  };
  ...
  const handleStartFromFirst = () => {
    ...
    if (firstLesson) {
      navigate(`/cto/training/lessons/${firstLesson.id}`);
    }
  };
  ...
  const handleResume = () => {
    ...
    if (incompleteLesson) {
      navigate(`/cto/training/lessons/${incompleteLesson.id}`);
    } else {
  ...
  return (
    <Stack gap="lg">
      <Group>
        <Button variant="subtle" leftSection={<IconArrowLeft size={16} />} onClick={() => navigate('/cto/training')}>
          Back to Training
        </Button>
      </Group>
...
export default CtoTrainingModuleDetail;
