...
interface GroceryHomeDashboardProps {
  restaurantId: string;
  restaurant?: any;
  readiness?: any;
...
export const GroceryHomeDashboard = ({
  restaurantId,
  restaurant,
  readiness,
  labels,
}: GroceryHomeDashboardProps) => {
    ...
    try {
      ...
      const lowStock =
        products?.filter(
          (p) =>
            p.is_available &&
            50 - (p.order_count || 0) > 0 &&
            50 - (p.order_count || 0) <= 10
        ).length || 0;
      const outOfStock =
        products?.filter(
          (p) => p.is_available && 50 - (p.order_count || 0) <= 0
        ).length || 0;
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="border-orange-200 dark:border-orange-900 bg-orange-500/10">
          <CardHeader>
            <CardTitle className="text-orange-900 dark:text-orange-100">
              Complete your {labels.entityLabel} setup
            </CardTitle>
          </CardHeader>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            >
              ...
              <span className="text-sm">
                Manage {labels.catalogLabel.toLowerCase()}
              </span>
            </Button>
            <Button
              variant="outline"
            ...
            >
              ...
              <span className="text-sm">{labels.availabilityLabel}</span>
            </Button>
            <Button
              variant="outline"
              className="h-auto flex-col py-4"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Top {labels.itemNounPlural}</CardTitle>
          </CardHeader>
          <CardContent>
            {topProducts.length === 0 ? (
              <div className="h-[250px] flex items-center justify-center text-muted-foreground">
                No {labels.itemNoun} data yet
              </div>
            ) : (
      ...
      <Card className="border-dashed border-2 border-muted">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Upload a file or connect your POS to keep {labels.itemNounPlural}{" "}
            and stock levels in sync. This helps reduce out-of-stock orders and
