...
const OrdersAuth = lazy(() => import("@tablet/OrdersAuth"));
const LiveOrdersWorkspace = lazy(() => import("@tablet/LiveOrdersWorkspace"));
...
const App: React.FC = () => {
  useEffect(() => {
    document.body.classList.add("orders-tablet-app");
    const lockOrientation = () => {
      try {
        const anyScreen = window.screen as unknown as { orientation?: { lock?: (v: string) => Promise<void> } };
    ...
    return () => {
      document.body.classList.remove("orders-tablet-app");
      ...
      <ErrorBoundary>
        <QueryClientProvider client={queryClient}>
          <Suspense fallback={<RouteFallback />}>
            <Routes>
              <Route path="/restaurant/auth" element={<OrdersAuth />} />
              <Route path="/live-orders" element={<LiveOrdersWorkspace />} />
