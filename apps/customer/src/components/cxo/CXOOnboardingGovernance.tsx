...
import { DocumentList } from '@/lib/cxo/components/DocumentList';
import { DocumentViewer } from '@/lib/cxo/components/DocumentViewer';
...
export const CXOOnboardingGovernance: React.FC = () => {
  ...
  const loadDocuments = async () => {
    setLoading(true);
    try {
      const indexResponse = await fetch('/lib/cxo/metadata/document_index.json');
  ...
  return (
    <Stack gap="lg">
      <div>
        <Title order={2}>CXO Onboarding & Governance</Title>
        <Text c="dimmed" size="sm">
          Central hub for CXO role onboarding, governance, policies, workflows, and system access.
        </Text>
      </div>
      ...
      {allAcknowledged ? (
        <Alert
          ...
          color="green"
        >
          You have successfully acknowledged all required CXO governance documents. You have full access to the CXO portal.
        </Alert>
      ) : (
        <Alert
          ...
          color="orange"
        >
          You must acknowledge all CXO governance documents before gaining full access to operational tools and dashboards.
          <br />
...
export default CXOOnboardingGovernance;
