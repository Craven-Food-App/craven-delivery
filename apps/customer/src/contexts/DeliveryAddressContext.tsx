...
interface DeliveryAddressContextType {
  /** Currently selected delivery address (from header or any selector). */
  selectedAddress: DeliveryAddressRecord | null;
  /** Set the selected delivery address (used by header and other selectors). */
  setSelectedAddress: (address: DeliveryAddressRecord | null) => void;
  /** Display string for the address (e.g. "123 Main St, City"). Never hardcoded—from user selection or saved addresses only. */
...
export const DeliveryAddressProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const value: DeliveryAddressContextType = {
    selectedAddress,
    setSelectedAddress,
