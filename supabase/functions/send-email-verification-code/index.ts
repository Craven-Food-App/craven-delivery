...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[send-email-verification-code] Received request for email: ${email}`);
    ...
    console.log(`[send-email-verification-code] Generated code for ${email}`);
    ...
    // Also clean up expired verifications
    await supabase
      .from("phone_verifications")
      .delete()
      .lt("expires_at", now);

    // Insert verification record with step 2 (6-digit code)
    let insertData: any = {
      phone: "0000000000", // Dummy phone since we only need email for login
      email,
      code: sixDigitCode,
      expires_at: new Date(Date.now() + 10 * 60 * 1000).toISOString(), // 10 minutes
      step: 2, // Directly to step 2 (6-digit code) - matches verify-phone-code step 2
    };
    ...
    // If step column doesn't exist, try without it
    if (insertError && (insertError.message?.includes("step") || insertError.code === "42703")) {
      console.log("[send-email-verification-code] Step column not found, inserting without step field");
    ...
    if (insertError) {
      console.error("[send-email-verification-code] Database insert error:", insertError);
              ...
              <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                ...
                <tr>
                  <td style="background: linear-gradient(135deg, #ff6b00 0%, #ff8c00 100%); padding: 40px; text-align: center; border-radius: 8px 8px 0 0;">
                    <h1 style="margin: 0; color: #ffffff; font-size: 32px; font-weight: bold;">Your Sign-In Code</h1>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding: 40px;">
                    <p style="margin: 0 0 20px 0; color: #4a4a4a; font-size: 16px; line-height: 1.6;">
                      Enter this verification code to sign in to your Feeder account.
                    </p>
                    ...
                    <p style="margin: 20px 0; color: #4a4a4a; font-size: 14px; line-height: 1.6;">
                      Return to the sign-in page and enter this 6-digit code to complete your login.
                    </p>
    ...
    try {
      ...
      console.log(`[send-email-verification-code] Email sent to ${email}:`, emailResponse);

      if (emailResponse.error) {
        console.error("[send-email-verification-code] Resend API error:", emailResponse.error);
        throw new Error(`Failed to send email: ${emailResponse.error.message || JSON.stringify(emailResponse.error)}`);
      }
    } catch (emailError: any) {
      console.error("[send-email-verification-code] Error sending email:", emailError);
