...
// CORS helper (inline - no shared import needed for dashboard deployment)
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🔍 Looking for user:', { recipient_user_id, recipient_email });

    // Find the user
    let userId = recipient_user_id;
    if (!userId && recipient_email) {
      console.log('🔎 Searching for user by email:', recipient_email);
      const { data: user, error: userError } = await supabaseAdmin.auth.admin.getUserByEmail(recipient_email);
      if (userError) {
        console.error('❌ Error fetching user by email:', userError);
        return new Response(
          JSON.stringify({ error: `Error finding user: ${userError.message}` }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      if (!user?.user) {
        console.log('❌ User not found by email:', recipient_email);
        return new Response(
    ...
    console.log('🔍 Searching for grants for user:', userId);
    ...
    console.log('📊 Found grants:', grants?.length || 0, grants);

    if (!grants || grants.length === 0) {
      console.log('❌ No grants found for user:', userId);
      
      // Check if grants were already revoked
      const { data: cancellations } = await supabaseAdmin
        .from('equity_ledger')
        .select('shares_amount, transaction_date')
        .eq('recipient_user_id', userId)
        .eq('transaction_type', 'cancellation');
      
      if (cancellations && cancellations.length > 0) {
        const totalRevoked = cancellations.reduce((sum, c) => sum + Number(c.shares_amount || 0), 0);
        console.log('ℹ️ Grants already revoked:', totalRevoked);
        return new Response(
          JSON.stringify({ 
            error: 'No active grants found for this user',
            message: `This user's grants have already been revoked (${totalRevoked.toLocaleString()} shares revoked)`,
            already_revoked: true,
            shares_already_revoked: totalRevoked
          }),
          { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      return new Response(
        JSON.stringify({ 
          error: 'No grants found for this user',
          user_id: userId,
          message: 'No equity grants found in the ledger for this user'
        }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
